...
async function extractor(
  ...
  switch (options.framework) {
    case Frameworks.React: {
      ...
      const appTsxFiles = getNodesFiles(
        depGraph,
        context.projectName,
        '.tsx',
        '.spec'
      );
      const projectDepsTsxFiles = getProjectDepsFiles(
        depGraph,
        projectDeps,
        '.tsx',
        '.spec'
      ...
      const transUnitsApp = elementsApp.filter(
        (i: any) => i.type === 'TransUnit'
      );
      const pluralsApp = elementsApp.filter((i: any) => i.type === 'Plural');
      const elementsProjectDeps = (extractTranslateElements(
        projectDepsTsxFiles
      ) as any).flat();
      const transUnitsProjectDeps = elementsProjectDeps.filter(
        (i: any) => i.type === 'TransUnit'
      );
      const pluralsProjectsDeps = elementsProjectDeps.filter(
        (i: any) => i.type === 'Plural'
      );
        ...
        try {
          ...
          logger.info(
            translations
              ...
              : `Translations founded. Updating messages file`
          );
          const translationsUnitsApp = manageTrans(transUnitsApp, translations);
          const translationsUnitsProjectDeps = manageTrans(
            transUnitsProjectDeps,
            translations
          );
          const translationsPluralsApp = managePlural(pluralsApp, translations);
          const translationsPluralsProjectsDeps = managePlural(
            pluralsProjectsDeps,
            translations
          );
          writeTranslationFile(
            options.directory,
            {
              ...translationsUnitsApp,
              ...translationsUnitsProjectDeps,
              ...translationsPluralsApp,
              ...translationsPluralsProjectsDeps,
            },
            locale
          );
          logger.fatal(
            `Locales were save at: ${options.directory}/messages.${locale}.json`
          );
        } catch (e) {
          logger.error(e.message);
          logger.error(
            `Please check that the file is not empty or contains some syntax errors`
          );
        }
      });
